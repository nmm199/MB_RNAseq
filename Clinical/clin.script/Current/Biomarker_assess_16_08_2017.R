...
for (gene in 1:length(gene_list)){
  ...
  #### Can create a list of data to exclude form the chi.sq test but will need to amend loop so easier to run all
  #exclude <- c("NMB", "Relapsetodeath", "Event", "EFS", "Followup", "OS.cat", "age.cont", "age.cat.adult.16", "age.cat.adult.21")
  exclude <- c("NMB", "Event", "EFS", "Followup", "OS.cat", "age.cont", "age.cat.adult.16", "age.cat.adult.21")
  ...
  for(v in 7:length(variable_list2)){  ### was (variable_list)
    ...
    tryCatch({ #### This catches the error and outputs it to screen but allows the program to continue running
      ...
      assign(paste0("chi_res_",variable_list2[[v]]), res)   ### was (variable_list)
  ...
  pval_chi_results <- list()
  #adj_pval_chi_results <- list()  ### added 15/8/17
  table_chi_results <- data.frame()
  #### create a matrix of defined size to hold the chisquare test results
  m <- matrix(0, ncol = 35, nrow = 1)  ### this has not been updated in this version, however it was updated in the 100817v2 version from 25 to 36
  ...
  for (c in 1:length(chsqnm)){
    ...
    #sig.p.adjust <- chi.sq.results[[c]][[1]]$p.adjust
    #tbl <- chi.sq.results[[c]][[2]]   ### have hashed this out as table_chi_results is not meaningful
    #tbl_df <- as.data.frame(tbl)     ### have hashed this out as table_chi_results is not meaningful
    #### add the result from each chi square test to a list 
    #significant_chi_results <- append(significant_chi_results, sig) 
    pval_chi_results <- append(pval_chi_results, sig) ### rename as pval_chi_results to avoid confusion with naming of significant_chisquare_results
  ...
  chisqout <- as.data.frame(do.call(cbind, pval_chi_results))
  #chisqout <- as.data.frame(do.call(cbind, pval_chi_results,adj_pval_chi_results)) ### added adj_pval_chi_results but did not work
  ...
  ##################################
  ### logistic regression - updated script per 100817v2 update that was functional
  ...
  for(t in 3:length(subset_df)){
    ...
    assign(paste0("pairwise_t_test",test_factors[[t]]), pairwise)  ### changed regression to pairwise
  }
  ...
  for (cox in 1:length(Output_Cox)){
    ...
    cox.n <- summary(Output_Cox[[cox]])$n   ### why does this not have a subset
  ...
  rownames(COX_OUT_DF) <- gene_list[[gene]] ### keep this line in
  assign(paste0("COX_data_frame_",gene_list[[gene]], cox_names[[cox]]), COX_OUT_DF)
  
  ### currently ALL_COX_Results is outputting cox regression for relapse in G3G4 only
  ### therefore subsequent most_significant_Cox is for relapse in G3G4 
  
  ALL_COX_Results <- as.list(mget(ls(pattern="^COX_data_frame")))
  COX_DF <- as.data.frame(do.call("rbind",ALL_COX_Results)) ### this line added 16/8/17
  ...
  #most_significant_Cox <- COX_DF[which(COX_DF$cox.pvalue < 0.05),]   ### this single line has replaced the for loop above that creates most_significant_Cox, however both only produce G3G4 relapse data
  
  
  #########################################
  ### logistic regression dataframe
  ### updated below 160817
...
### convert any dataframes with a list as a vector within one of the columns to a dataframe that can be written to csv (example below)
### the following dataframe cannot be used in interpretation as there is a problem with significant_chisquare_results_df
### new_significant_chisquare_results_df <- data.frame(lapply(significant_chisquare_results_df, as.numeric), stringsAsFactors=FALSE)  ### DO NOT USE, cannot interpret
### write.csv(file="/home/nmm199/R/MB_RNAseq/Clinical/clin.results/Sig_chisq160817.csv", new_significant_chisquare_results_df, row.names=TRUE)  ### DO NOT USE, cannot interpret
