...
#foreach (gene = 1:length(gene_list))%dopar%{
  ...
  #foreach(v = 1:length(matched.test.pData))%dopar%{
  for(v in 32:length(matched.test.pData)){
  ...
  for (c in 1:length(chsqnm)){
    ...
    sig <- chi.sq.results[[c]][[1]]$p.value
	  #sig.p.adjust <- chi.sq.results[[c]]$p.adjust
    ...
    #adjusted_significant_chi_results <- append(significant_chi_results, sig.p.adjust)
    table_chi_results <- rbind(table_chi_results, tbl_df)
  }
 
  #### turn the list into a data frame 
  chisqout <- as.data.frame(do.call(cbind, significant_chi_results))
  #chisqout <- as.data.frame(do.call(cbind, adjusted_significant_chi_results))
...
#  }
  ...
  chsq_significant_in_sexes <- list()
  for (rw in 1:length(Chi_squared_results_df$chi_res_sex)){
    if(Chi_squared_results_df$chi_res_sex[[rw]] < 0.05){
      sig <- Chi_squared_results_df$chi_res_sex[[rw]]
      names(sig) <- rownames(Chi_squared_results_df[rw, ])
      chsq_significant_in_sexes <- append(chsq_significant_in_sexes, sig)
    }
  }
  ...
  ### TERT
  cat (paste("Extracting significant results for TERT status from chi squared tests",gene_list[[gene]]), sep ="\n")
...
}  ### to end first loop running through genes
