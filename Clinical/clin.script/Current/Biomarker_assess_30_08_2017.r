...
load("/home/nmm199/R/MB_RNAseq/pData_saved.RData")
...
#gene_list <- as.list("ENSG00000000003.14_1")
#### check a small list of specific gene ids
#gene_list <- as.list(c("ENSG00000001036", "ENSG00000001497", "ENSG00000002016"))
#### check a larger list of genes 
...
#gene_list <- as.list(head(rownames(mb.vsd), 10))
#### runs the first 40 gene ids in the list
gene_list <- as.list(head(rownames(mb.vsd), 300))
#### remove the dot and trailing numbers from the gen ids, not needed
#gene_list <- gsub("\\..*","",gene_list)
#### Removes duplicate gene ids from the list of IDs being tested, not needed
...
for (gene in 1:length(gene_list)){
  ...
  #subset_df <- matched.test.pData[c(7,8,10,11,13,15,16,17,18,26,14,19,20,21,22,23,24,25,27,28,34,35)]  ## rm 12
  ...
  #for(v in 1:length(matched.test.pData)){
  for(v in 1:length(MATCHED_TEST_PDATA[[gene]])){
    #for(v in 7:length(variable_list)){
    #foreach(v = 1:length(test_factors))%dopar%{
    #for(v in 1:length(subset_df)){
    ...
    tryCatch({ #### This catches the error and outputs it to screen but allows the program to continue running
      ...
      #res <- chi.sq(na.omit(MATCHED_TEST_PDATA[[gene]][, v]), as.data.frame(CAT_SEQ_LIST[[gene]][!is.na(MATCHED_TEST_PDATA[[gene]][, v])]))  
      res <- chi.sq(na.omit(MATCHED_TEST_PDATA[[gene]][, v]), as.data.frame(t(CAT_SEQ_LIST[[gene]][!is.na(MATCHED_TEST_PDATA[[gene]][, v])])))  
      #res <- chi.sq(na.omit(subset_df[[gene]][, v]), CAT_SEQ_LIST[[gene]][!is.na(subset_df[[gene]][, v])])  
      ...
      #assign(paste0("chi_res_",variable_list[[v]]), res)
      assign(paste0("chi_res_",colnames(MATCHED_TEST_PDATA[[gene]][v])), res)
    }, error=function(e){cat("ERROR :",conditionMessage(e), "\n")})  #### prints the error message to screen
  }
  #### create a list containing all of th e results of the chi.squared test
  cat (paste("creating chi.sq.results ",gene_list[[gene]]), sep ="\n")
  ...
  #colnames(Chi_sq_sig_df) <- test_factors
  ...
  for (c in 1:length(chsqnm)){
    ...
    #sig.p.adjust <- p.adjust(sig, method = "BH") 
  ...
  #colnames(chisqout) <- test_factors
  ...
  #Chi_squared_results_df <- as.data.frame(rbind(Chi_squared_results_df, chisqout))
  ...
  #df_sig_chisq <- do.call(rbind, significant_chisquare_results)
  ...
  for (rw in 1:ncol(significant_chisquare_results_df)){
  ...
  subset_df <- MATCHED_TEST_PDATA[[gene]][c(7,8,10,11,13,15,16,17,18,26,14,19,20,21,22,23,24,25,27,28,34,35)]  ## rm 12
  #### create a vector of test factors
  test_factors <- as.vector(names(subset_df))
  ...
  #sink()
  #dev.off()
  cat (paste("Extracting biomarkers associated with 5 years survival ",gene_list[[gene]]), sep = "\n")
