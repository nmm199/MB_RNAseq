...
#### basic function providing a putative biomarker
...
### is there another way to subset to include values within a range, using similar to below
# age.cont.incl <- subset(pData, pData$Age_R>3&pData$Age_R<16)

### metastatic
mstatus <- pData$Mstatus_R
#### I wonder if there is room for increased categories
...
message("reading out the summary statistics for the entire cohort n=802")
...
message("age summary statistics n=802")
...
### summary data on included group

### summary data for all variables within the age 3-16 yo group
...
### Chi squared tests and function


### create list for the output for each variable
### use temporary table.temp within the function, and to replace x, y for each new equation
...
### chi squared tests required for analyses with generated lists

### if an error occurs in the above function, ensure that chi.test.temp.stat has been used
...
message("correlation coefficients for association between variables")
...
cor.test(matched.test.pData$age.cont, matched.goi.vsd)
...
# cor.test(matched.test.pData$age.cont, matched.goi.vsd, method = 'spearman')
...
message ("confirm that variances are equal, ie. leveneTest is not significant")
