...
### This is a list of functions that were written for the "clinical_data_goi.R" script analysis
...
## x <- variable 1
## y <- variable 2
## Title <- main title for heatmap, currently returning error message therefore removed from function
...
#chi.sq <- function(x,y, Title){
 ...
 # aheatmap(chi.test.temp$residuals, Rowv=NA, Colv = NA, main = Title)
...
### Function Number 8
### Function entitled "updatepData" which creates the relevant dataframe from updated clinical data. 
### Input
## x.data, which is the clinical database
## meth7, which is the 7 molecular group database
## cytogen, which is the cytogenetic arm calls file

### output
## "test.pData" with all the variable names that can be then directly imported to run with R script in "clinical_data_goi.R"
## also provides some summary statistics on the cohort including age
...
updatepData <-  function(x.data, meth7, cytogen, pdf.file = NULL, log.file = NULL){
  ...
  #age.filter <- ifelse(age.filter =="TRUE", "age 3-16", "outside age range")
  ...
  cytogen.q13.cat <- ifelse(cytogen$q13 == "Loss", "13q Loss", "no 13q loss") 
  ...
  # curative <- ifelse(curative =="Yes", "Curative intent", "Non curative")
  ...
  cat ("analysing age as qq plot, see PDF, and then summary characteristics for age (continuous)", sep ="\n" )
...
### Function Number 9
### Function called "clinPathAssess", which is designed to produce chi-squared results, correlations, univariate logistic and cox regression for a biomarker of interest
### input
## "test.pData" which is the combined dataframe which is created from Function Number 8 above
## "goi.vsd", which is the file containing expression data for goi
...
clinPathAssess <- function(test.pData,
  ...
  #matched.test.pData$age.filter <- ifelse(matched.test.pData$age.filter =="TRUE", "age 3-16", "outside age range")
  ...
  ### chi squared function with aheatmap titles, which did not work when the whole file was run
  
  #list.age.cat.infant <- chi.sq(x = matched.test.pData$age.cat.infant, y = matched.goi.vsd.cat, Title = "Infant MB (True vs False) and Biomarker expression (Low vs High)")
  
  #list.sex <- chi.sq (x = matched.test.pData$sex, y= matched.goi.vsd.cat, Title = "Gender and Biomarker expression (Low vs High)" )
  
  #list.mstatus <- chi.sq(x = matched.test.pData$mstatus, y=matched.goi.vsd.cat, Title = "Metastatic status (M0/M1 vs M+) and Biomarker Expression (Low vs High)") 
  
  #list.relapse <- chi.sq(x = matched.test.pData$relapse, y = matched.goi.vsd.cat, Title = "Relapse status and Biomarker expression (Low vs High) ") 
  
  #list.resection <- chi.sq (x = matched.test.pData$resection, y = matched.goi.vsd.cat, Title = "Resection status and Biomarker expression (Low vs High)" )
  
  #list.meth.4 <- chi.sq (x= matched.test.pData$meth, y = matched.goi.vsd.cat, Title = "Methylation status and Biomarker expression (Low vs High) ")
  
  #list.meth.7 <- chi.sq (x = matched.test.pData$meth7, y = matched.goi.vsd.cat, Title = "Methylation status and Biomarker expression (Low vs High) ")
  
  #list.path <- chi.sq (x = matched.test.pData$histopath, y = matched.goi.vsd.cat, Title = "Histopathology and Biomarker expression (Low vs High)" )
  
  #list.LCA <- chi.sq (x = matched.test.pData$LCA, y = matched.goi.vsd.cat, "Histopathology and Biomarker expression (Low vs High)" )
  
  #list.MYC <- chi.sq (x = matched.test.pData$MYC.cat, y = matched.goi.vsd.cat, Title = "MYC amplification and Biomarker expression (Low vs High)")
  
  #list.MYCN <- chi.sq (x = matched.test.pData$MYCN.cat, y = matched.goi.vsd.cat, Title = "MYCN amplification and Biomarker expression (Low vs High)")
  
  #list.MYCMYCN <- chi.sq(x= matched.test.pData$MYCMYCN.cat, y= matched.goi.vsd.cat, Title = "MYC/MYCN amplification and Biomarker expression (Low vs High)")
  
  #list.TP53 <- chi.sq (x = matched.test.pData$TP53.cat, y = matched.goi.vsd.cat, Title = "TP53 mutation status and Biomarker expression (Low vs High)")
  
  #list.TERT <- chi.sq (x = matched.test.pData$TERT.cat, y = matched.goi.vsd.cat, Title = "TERT mutation status and Biomarker expression (Low vs High)")
  ...
  cat ("summary statistics G3, aged 3-16 years", sep = "\n")
  ### G3, aged 3-16 years
  ...
  cat ("summary statistics G4, aged 3-16 years", sep = "\n")
  ### G4, aged 3-16 years
  ...
  cat ("summary statistics SHH, aged 3-16 years", sep = "\n")
  ### SHH, aged 3-16 years
  ...
  cat ("summary statistics WNT, aged 3-16 years", sep = "\n")
  ### WNT, aged 3-16 years
  ...
  cat ("creating binary relapse variables labelled 0,1 for event analysis for entire survival cohort and for G3G4 combined group", sep = "\n")
